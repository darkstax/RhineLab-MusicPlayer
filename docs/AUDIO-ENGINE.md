...
| **M4** | **（09-14 用户在场窗口解锁，主进程亲自实施——交互密集）** 独占 + 协商状态机 + 降级链 + 300ms 升档 + 热插拔 | 设计 §7.2；验收=用户实时确认 |
...
- **M4 独占模式**：✅ 09-13 排除自主循环 → **09-14 用户在场窗口解锁**，主进程亲自实施，
  交互确认（拔插/抢占/听感）由用户实时承担；范围=§7.2 协商降级链+300ms 升档+热插拔+位完美徽章。
- **M2 听感**：✅ 09-14 用户亲测"质量不错"，停点解除（M2-FINDINGS §8）。
