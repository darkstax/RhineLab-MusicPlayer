...
> 状态：**已批准（09-13 用户：M5d-Q1/Q2、M5-Q3 通过）——待开工** · 2026-09-13
...
- **M5v2-Q1**：✅ **已批准（09-13 用户）**——M5d 允许修改上游 `src/scene.ts`/`main.ts`（封面材质槽、
  DOM 骨架/字段表/页签/浮层）；"上游零侵入"铁律对 M5d 显式豁免，改动仍须 reviewer 逐行把关
  且不破坏 boot/档案阵列既有行为（回归底线不变）。
- **M5v2-Q2**：✅ **已批准（09-13 用户）**——"阵列卡片素面、仅选中卡贴封面"定案；
  整墙图集方案（建议-5）不立项。
- **M5v2-Q3**：✅ **已批准（09-13 用户）**——三路径检索路由（≥3 字 trigram / 拉丁 unicode61 /
  <3 字 CJK LIKE）为 v1 承诺；拼音检索不承诺（建议-1）。
- **M5v2-Q4**：默认采纳 v2 建议（随 M5a 执行，各自独立 commit 便于回滚）；用户未异议。
- **M5v2-Q5**：默认采纳 v2 建议（小窗归 M6）；用户未异议。
